...
Sistema completo para controlar un brazo robótico de cuatro servomotores usando **ROS 2**, una **ESP32** y un **mando Xbox 360**. El proyecto permite operar el brazo mediante teclado o joystick con control incremental por grados.
...
Este proyecto implementa el control completo de un brazo robótico de 4 DOF mediante la integración de **ROS 2 Jazzy** y una **ESP32**. A través de nodos Python y comunicación serial, se controlan cuatro servomotores: base, hombro, codo y muñeca.

El sistema incluye **dos modos de operación**:

- **Modo teclado** — control incremental por ángulos (±10°) usando las teclas `W A S D Q E Z C`.
...
Nodo `node_connection` que se suscribe al tópico `/pub` y reenvía los ángulos de los 4 servos a la ESP32 por serial en el formato:
...
[Nodo ROS 2: control_teclado.py / xbox_control.py]
   ...
   con servo1=linear.x, servo2=linear.y,
       servo3=linear.z,  servo4=angular.x
        │
        ▼
[Nodo ROS 2: connect.py]
   Se suscribe a /pub
   Serializa → "s1,s2,s3,s4\n"
